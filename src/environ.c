...
//  { "XYMONSERVERWWWNAME", XYMONHOSTNAME },
 ...
 // { "XYMONHOME", BUILD_HOME },
    ...
    { "MAIL", "$MAILC -s" },
    { "SVCCODES", 
      "disk:100,cpu:200,procs:300,svcs:350,msgs:400,conn:500,http:600,dns:800,smtp:725,telnet:723,ftp:721,pop:810,pop3:810,pop-3:810,ssh:722,imap:843,ssh1:722,ssh2:722,imap2:843,imap3:843,imap4:843,pop2:809,pop-2:809,nntp:819,test:901" },
    ...
    { "TEST2RRD", 
      "cpu=la,disk,memory,$PINGCOLUMN=tcp,http=tcp,dns=tcp,dig=tcp,time=ntpstat,vmstat,iostat,netstat,temperature,apache,bind,sendmail,nmailq,socks,bea,iishealth,citrix,xymongen,xymonnet,xymonproxy,xymond" },
    { "GRAPHS", 
      "la,disk:disk_part:5,memory,users,vmstat,iostat,tcp.http,tcp,netstat,temperature,ntpstat,apache,bind,sendmail,nmailq,socks,bea,iishealth,citrix,xymongen,xymonnet,xymonproxy,xymond" },
    ...
    { "XYMONPAGEROWFONT",
      "SIZE=\"+1\" COLOR=\"#FFFFCC\" FACE=\"Tahoma, Arial, Helvetica\"" },
    ...
    { "XYMONNETSVCS", 
      "smtp telnet ftp pop pop3 pop-3 ssh imap ssh1 ssh2 imap2 imap3 imap4 pop2 pop-2 nntp" },
    ...
    { "XYMONBODYFOOTER", "" },
    { "XYMONALLOKTEXT", 
      "<FONT SIZE=+2 FACE=\"Arial, Helvetica\"><BR><BR><I>All Monitored Systems OK</I></FONT><BR><BR>" },
    { "HOSTPOPUP", "CDI" },
    { "STATUSLIFETIME", "30" },
    { NULL, NULL }
};
...
//MEMDEFINE(l);
    ...
    while (stackfgets(inbuf, NULL)) 
    {
      ...
      if (oneenv) 
      {
        ...
        if (appendto) 
        {
          ...
          {
            // oneenv is now VARxxVALUE, so fix it to be a 
            // normal env. variable format 
            strcat(oneenv, "=");
  ...
  {
    logger( LSTD_OPEN_FILE_FAILED, envfile ) ;
